...
function processAssets(compilation, options) {
    ...
    const promises = uncachedAssets.map((assetName, index) => {
        ...
        return minify(assetName, asset, useSourceMaps, worker, options.cacheDir, optionsWithoutCacheDir)
            .then(msgContent => {
                if(msgContent.map) {
                    assetHash[assetName] = new SourceMapSource(msgContent.source, msgContent.name, msgContent.map); // eslint-disable-line
                                                                                                                    // no-param-reassign,
                                                                                                                    // max-len
                }
                else {
                    assetHash[assetName] = new RawSource(msgContent.source); // eslint-disable-line no-param-reassign,
                                                                             // max-len
                }
            }).catch((e) => {
